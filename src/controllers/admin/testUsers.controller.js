...
// Sample data pools
const firstNames = {
	MALE: ['Alex', 'Ben', 'Chris', 'David', 'Ethan', 'Frank', 'George', 'Henry', 'Ian', 'Jack', 'Kevin', 'Luke', 'Mark', 'Nathan', 'Oscar'],
	FEMALE: ['Alice', 'Beth', 'Carol', 'Diana', 'Emma', 'Fiona', 'Grace', 'Hannah', 'Iris', 'Julia', 'Kate', 'Luna', 'Mia', 'Nina', 'Olivia']
};

const lastNames = ['Anderson', 'Brown', 'Clark', 'Davis', 'Evans', 'Garcia', 'Harris', 'Johnson', 'Jones', 'Lee', 'Martinez', 'Miller', 'Rodriguez', 'Smith', 'Taylor', 'Thomas', 'White', 'Wilson', 'Young'];
...
const generateRandomUser = () => {
	...
	const username = `${firstName.toLowerCase()}_${lastName.toLowerCase()}_${getRandomInt(100, 999)}`;
	const email = `${firstName.toLowerCase()}.${lastName.toLowerCase()}${getRandomInt(100, 999)}@testuser.com`;
	...
	return {
		...
		gender,
		dateOfBirth: new Date(getRandomInt(1970, 2005), getRandomInt(0, 11), getRandomInt(1, 28)),
...
const generateTestUsers = AsyncHandler(async (req, res) => {
	...
	if (process.env.NODE_ENV === 'production') {
		return res.status(403).json(
			new ApiResponse(403, null, 'Test user generation not allowed in production')
		);
	}
	...
	try {
		...
		if (illnesses.length === 0) {
			return res.status(400).json(
				new ApiResponse(400, null, 'No illnesses found in database. Please seed illnesses first.')
			);
		}
		...
		for (let i = 0; i < count; i++) {
			...
			const illnessPreferences = generateRandomIllnessPreferences(illnesses, illnessCount);
		...
		return res.status(500).json(
			new ApiResponse(500, null, 'Error generating test users')
		);
	}
});
...
const getTestUsersStats = AsyncHandler(async (req, res) => {
	try {
		...
		return res.status(200).json(
			new ApiResponse(200, {
				...
				sampleUsers: sampleUsers.map(user => ({
					...
					illnesses: user.preferences?.illnesses?.map(ip => ({
		...
		return res.status(500).json(
			new ApiResponse(500, null, 'Error retrieving statistics')
		);
	}
});
...
const clearTestUsers = AsyncHandler(async (req, res) => {
	...
	if (process.env.NODE_ENV === 'production') {
		return res.status(403).json(
			new ApiResponse(403, null, 'Test user clearing not allowed in production')
		);
	}
	...
	try {
		...
		return res.status(500).json(
			new ApiResponse(500, null, 'Error clearing test users')
		);
	}
});
