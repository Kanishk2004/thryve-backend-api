...
export class MatchingService {
	...
	static async findMatches(userId, filters = {}) {
		...
		if (!currentUser?.preferences) {
			throw ApiError.badRequest(
				'Please set your preferences first to find matches'
			);
		}
		...
		const matchesWithScores = this.calculateCompatibilityScores(
			currentUser,
			potentialMatches
		);
	...
	static buildMatchingCriteria(currentUser, filters) {
		...
		const criteria = {
			AND: [
				...
				...(userPrefs.preferredGender !== 'ANY'
					? this.buildGenderFilter(userPrefs, filters)
					: []),

				// Illness overlap filtering
				...(userPrefs.illnesses.length > 0
					? this.buildIllnessFilter(userPrefs, filters)
					: []),
				...
				...(filters.isOnline
					? [
							{
								/* Add online status when implemented */
							},
					  ]
					: []),
			],
	...
	static buildGenderFilter(userPrefs, filters) {
		...
		if (targetGender && targetGender !== 'ANY') {
			return [
				{
					OR: [{ gender: null }, { gender: targetGender }],
				},
			];
		}
	...
	static buildIllnessFilter(userPrefs, filters) {
		...
		if (!targetCategories) {
			...
			targetCategories = [
				...new Set(userPrefs.illnesses.map((ui) => ui.illness.category)),
			];
		}
	...
	static calculateCompatibility(user1, user2) {
		...
		if (user1.preferences?.illnesses && user2.preferences?.illnesses) {
			const commonIllnesses = this.findCommonIllnesses(
				user1.preferences.illnesses,
				user2.preferences.illnesses
			);
			...
			if (commonIllnesses.length > 0) {
				reasons.push(
					`${commonIllnesses.length} shared health condition${
						commonIllnesses.length > 1 ? 's' : ''
					}`
				);
		...
		if (
			user1.preferences?.isSeekingMentor &&
			user2.preferences?.isOpenToMentoring
		) {
			score += 15;
	...
	static async recordInteraction(
		...
		console.log(
			`Interaction recorded: ${userId} -> ${targetUserId} (${interactionType})`
		);
